...
import { wordListHard } from '../dictionary/WordListHard';

const GameHard = () => {
  const [boardData, setBoardData] = useState(JSON.parse(localStorage.getItem("board-data-3")));
  ...
  const resetBoard = () => {
    ...
    var wordIndex = Math.floor(Math.random()*wordListHard[String.fromCharCode(97 + alphabetIndex)].length);
    let newBoardData = {...boardData, "solution":wordListHard[String.fromCharCode(97 + alphabetIndex)][wordIndex],
                          "rowIndex": 0, "boardWords":[], "boardRowStatus":[], "presentCharArray":[],
                          "absentCharArray":[], "correctCharArray":[], "status":"IN_PROGRESS"};
    setBoardData(newBoardData);
    localStorage.setItem('board-data-3', JSON.stringify(newBoardData));
  }
  ...
  const handleKeyPress = (key) => {
    ...
    if(key === "ENTER"){
      if(charArray.length === 7) {
        ...
        if(!wordListHard[word.charAt(0)].includes(word)) {
          handleError();
      ...
      <div className='cube'>
        {[0, 1, 2, 3, 4].map((row, rowIndex) =>(
          <div className={`cube-row ${boardData && row===boardData.rowIndex && error && "error"}`} key={rowIndex}> {
            [0, 1, 2, 3, 4, 5, 6].map((column, letterIndex)=>(
