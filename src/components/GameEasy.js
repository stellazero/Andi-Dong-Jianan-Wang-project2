...
const GameEasy = () => {
  ...
  const resetBoard = () => {
    ...
    var wordIndex = Math.floor(Math.random()*wordListEasy[String.fromCharCode(97 + alphabetIndex)].length);
    let newBoardData = {...boardData, "solution":wordListEasy[String.fromCharCode(97 + alphabetIndex)][wordIndex],
  ...
  const handleKeyPress = (key) => {
    if(boardData.rowIndex > 7 || boardData.status === "WIN") return;
    if(key === "ENTER"){
      if(charArray.length === 5) {
        let word = charArray.join("").toLowerCase();
        if(!wordListEasy[word.charAt(0)].includes(word)) {
          handleError();
